...
Текст ошибки лежит в ключе `error`, реже в `message`. Пример истекшей сессии:

```json
{"result":"FAIL","type":"SESSION_EXPIRED","error":"Сессия истекла. Пожалуйста, попробуйте обновить страницу."}
```

Цикл повторных запросов ограничен десятью итерациями, число задается через
`Config::setRetryLimit()`. Пауза между ними — через `Config::setRetryDelay()`,
по умолчанию одна секунда. Если после всех попыток сайт так и отдает RID,
бросается исключение, а не пустой ответ
